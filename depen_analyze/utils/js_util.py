...
def get_initiators(har_log):
    ...
    for ini_item in har_log:
        try:
            ...
            continue
        if (
            "initiator" in json_obj["message"]["params"]
            and json_obj["message"]["params"]["initiator"]["type"] != "other"
        ):
            ini_list = []
            ...
            if not "stack" in json_obj["message"]["params"]["initiator"]:
                ...
                if not json_obj["message"]["params"]["initiator"]["url"].endswith(
                    ".js"
                ) and not target_url.endswith(".js"):
                    continue
                if len(ini_list) != 1:
                    initiators.append(ini_list)
            else:
                for frame in json_obj["message"]["params"]["initiator"]["stack"][
                    "callFrames"
                ]:
...
def get_compare_html(driver, url, js_url):
    ...
    driver.execute_cdp_cmd("Network.setBlockedURLs", {"urls": [js_url]})
    driver.get(url)
...
def get_js_dom_change_score(driver, url, js_url):
    ...
    diff_chg_count, diff_add_count, diff_sub_count = get_html_diff_cnt(
        html_origin, html_blcked
    )
    return (diff_chg_count + diff_add_count + diff_sub_count) / (
        diff_chg_count + diff_add_count + diff_sub_count + 1
    )
...
def get_xssed_domains(resp):
    ...
    try:
        for row in table.find_all("tr"):
            if row.find_all("img"):
                if len(row.find_all("img")) == 2 and row.find_all("img")[1].get(
                    "src"
                ).endswith("unfixed.gif"):
...
def get_bank_domain(bank_name):
    params = {"api_key": key.SERP_API_KEY, "q": bank_name, "gl": "cn", "hl": "zh-cn"}
    try:
